...
namespace MotionMod
{
    ...
    public partial class MotionWindow : Form
    {
        ...
        public event moveDelegate homeOver;
        ...
        public MotionWindow()
        {
            ...
            homeOver += homeOverPro;
            homeOver += enable_moveBtn;
        ...
        private Thread startPTPMove(int iAxisNO, int iDirect)
        {
            if (moveStarted != null)
                moveStarted();
            var t = new Thread(() =>
            {
                PTPMove_Axis(iAxisNO, iDirect);
                if (moveStopped != null)
                    moveStopped();
            });
            t.IsBackground = true;
        ...
        //暂时没有用到iDirection，不管点击正/负，都会移动到输入的位置
        private void PTPMove_Axis(int iAxisNO, int iDirection)
        {
            ...
            z_ptpprm.iCoorAxis = iAxisNO;
            double stepdis;
            bool ret = double.TryParse(stepText, out stepdis);
            if (!ret)
            {
                MessageBox.Show("请正确设置目标位置！");
            }
            else
            {
                z_ptpprm.dPos = stepdis;
                PTPPrm[] array_PTP = new PTPPrm[1];
                array_PTP[0] = z_ptpprm;

                bool flag = m_motion.Motion_PToPMove(array_PTP);
        ...
        public void homeOverPro()
        {
            if (InvokeRequired)
            {

                this.Invoke(new moveDelegate(homeOverPro));
                return;
            }

            if (radioButton_Move_H.Checked)
                BtnSetSpeed((int)velType.HighVel);
            else if (radioButton_Move_M.Checked)
                BtnSetSpeed((int)velType.midVel);
            else if (radioButton_Move_L.Checked)
                BtnSetSpeed((int)velType.lowVel);
        ...
        //速度选择按钮共用函数，注意！！！暂时改为了2，没有设置z轴速度，因为共享库中参数设置问题
